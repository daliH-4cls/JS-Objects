...
// DONE: Test working with objects here
let person = {name: "Scooby", age: 80};
console.log(person.name);
person.name = "Doo";
console.log(person.name);
person.snn = "560-08-7900";
console.log(person);

function processForm(event) {
    console.log("click");
    // DONE: prevent this function from reloading the page when the form is submitted
    event.preventDefault();

    // DONE: Create a newUser object that has all the user's info from the form
    let newUser = {
...
function addUser(user) {
    ...
    // DONE: Set the background color of the div to the user's favorite color
    div.style.backgroundColor = user.favColor;

    // Create a heading for the user's name
    let name = document.createElement("h2");
    // DONE: Set the text using the object data
